...
export const RouteItem = ({ route, nested = false, hasChildren = false, handleMenuClick = () => {} }: RouteItemProps) => {
    ...
    const item = (
        <StyledListItemButton isEnabled={route.isEnabled} sx={{ pl: nested ? 3 : 1 }} onClick={() => handleMenuClick(route)}>
            <ListItemIcon>
...
const StyledListItemButton = styled(ListItemButton)<{ isEnabled: boolean }>(({ theme, isEnabled }) => (!isEnabled ? { '*': { cursor: 'not-allowed', color: theme.palette.text.secondary } } : {}));
